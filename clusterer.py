...
def clusterer(basedir, g, PARADIGM):
	...
	if fnmatch.fnmatch(RAND,"randomized")==True:
		if fnmatch.fnmatch(PARADIGM,"milkshake")==True:
			for file in glob.glob(os.path.join(basedir,'level3_grace_edit','double_check','cope*_%s_randomized_tfce_corrp_*.nii.gz'%TYPE)):
				print(file)
				...
				cluster=subprocess.Popen(["/projects/niblab/modules/software/fsl/5.0.10/bin/cluster","-i",INPUT,"-t","0.95","-o",OUTPUT+"_cluster"],stdout=subprocess.PIPE)
